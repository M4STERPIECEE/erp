...
import com.erp.erp.domain.port.in.employee.*;
...
class EmployeeControllerTest {
    ...
    void should_create_employee_and_return_201() throws Exception {
        ...
        given(employeeWebMapper.toCommand(any(CreateEmployeeRequest.class))).willReturn(command);
        given(createEmployeeUseCase.create(any(CreateEmployeeCommand.class))).willReturn(result);
        given(employeeWebMapper.toResponse(any(EmployeeResult.class))).willReturn(responseDto);
    ...
    void should_get_employee_by_id_and_return_200() throws Exception {
        ...
        // when
        MockHttpServletResponse response = mockMvc
                .perform(get("/api/v1/employees/{id}", employeeId).accept(MediaType.APPLICATION_JSON))
