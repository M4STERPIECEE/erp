...
class AbsenceControllerTest {
    ...
    @BeforeEach
    void setUp() {
        absenceController = new AbsenceController(getAbsenceUseCase, getEmployeeByEmailUseCase, jwtTokenProvider);
        mockMvc = MockMvcBuilders.standaloneSetup(absenceController).setControllerAdvice(new GlobalExceptionHandler())
    ...
    void should_get_my_absences_and_return_200() throws Exception {
        ...
        given(getAbsenceUseCase.listEmployeeAbsences(anyLong(), anyInt(), anyInt()))
                .willReturn(Collections.emptyList());

        // when
        MockHttpServletResponse response = mockMvc
                .perform(get("/api/v1/absences/my-absences").accept(MediaType.APPLICATION_JSON))
